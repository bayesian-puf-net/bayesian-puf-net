...
df2=pd.read_csv('./Modeling_of_APUF_Compositions/64bit/HalfMillion/MPUF/mpuf.csv',header=None)

def create_model_conv():
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(30, name='fc1',
                              activation=tf.nn.relu),
        tf.keras.layers.Dense(30, name='fc2',
                              activation=tf.nn.relu),
        tf.keras.layers.Dense(15, name='fc3',
                              activation=tf.nn.relu),
...
def create_model_bayes(NUM_TRAIN_EXAMPLES):
    ...
    model = tf.keras.models.Sequential([
        tfp.layers.DenseFlipout(30, name='fc1',
...
X = np.array(df1.iloc[:139999,:65])
Y = np.array(df2.iloc[:139999,18:19])
...
np.savez('MPUF_result', acc, acc_bnn)
