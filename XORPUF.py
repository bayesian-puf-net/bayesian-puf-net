...
df2=pd.read_csv('./Modeling_of_APUF_Compositions/64bit/HalfMillion/5-XOR/5-xorpuf.csv',header=None)

def create_model_conv():
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(30, name='fc1',
                              activation=tf.nn.relu),
        tf.keras.layers.Dense(28, name='fc2',
                              activation=tf.nn.relu),
...
X = np.array(df1.iloc[:180999,:65])
Y = np.array(df2.iloc[:180999,:])
...
np.savez('5XORPUF_result', acc, acc_bnn)
