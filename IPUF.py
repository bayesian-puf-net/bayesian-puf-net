...
df2=pd.read_csv('./Modeling_of_APUF_Compositions/64bit/HalfMillion/IPUF_3_3/3_3testSetResponses_64_500000.csv',header=None)

def create_model_conv():
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(50, name='fc1',
                              activation=tf.nn.relu),
        tf.keras.layers.Dense(50, name='fc2',
                              activation=tf.nn.relu),
        tf.keras.layers.Dense(50, name='fc3',
                              activation=tf.nn.relu),
        tf.keras.layers.Dense(1, name='fc4',
...
def create_model_bayes(NUM_TRAIN_EXAMPLES):
    ...
    model = tf.keras.models.Sequential([
        tfp.layers.DenseFlipout(50, name='fc1',
...
X = np.array(df1.iloc[:299999,:65])
Y = np.array(df2.iloc[:299999,:])
...
np.savez('IPUF_result', acc, acc_bnn)
